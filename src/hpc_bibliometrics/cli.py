...
def collect(venue: str = typer.Argument(...), from_year: int = typer.Option(2016, "--from", "--from-year", min=1900),
    ...
    try:
        ...
        def report_year(item: YearResult) -> None:
            typer.echo(f"{item.year}: {item.count:4d} papers [{'cached' if item.cached else 'downloaded'}]")
...
def enrich(venue: str = typer.Argument(...), from_year: int = typer.Option(2016, "--from", min=1900),
           ...
           workers: int = typer.Option(4, "--workers", "-j", min=1, max=10), refresh: bool = typer.Option(False, "--refresh")) -> None:
    """Enrich a collected DBLP roster with OpenAlex affiliations by DOI."""
    try:
        get_venue(venue)
        typer.echo(f"Enriching {venue.upper()} {from_year}-{to_year} with OpenAlex using {workers} worker(s)...")
    ...
    typer.echo(f"Matched: {result.matched}; missing DOI: {result.missing_doi}; unmatched: {result.unmatched}")
